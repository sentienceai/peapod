...
/** A 1969 LCG. Small, exactly reproducible, and nobody has to trust a hash function for a
    dataset whose only job is to look plausible. @param {number} seed */
function rng(seed) {
  let s = seed % 233280;
  return () => {
...
/** Every symbol the generators below look up came out of ASSETS in the first place, so a miss
    is a bug in this file rather than a case a caller could hit — it throws instead of quietly
    substituting a stand-in asset, which would put invented prices on a real row.
    @param {string} symbol */
function knownAsset(symbol) {
  const a = asset(symbol);
  if (!a) throw new Error(`mock: no asset ${symbol}`);
  return a;
}
...
export function traderDetail(addr) {
  ...
  const tokenStats = row.tokens.map((/** @type {string} */ symbol) => {
    const trips = 1 + Math.floor(rand() * Math.max(2, row.roundTrips / row.tokens.length));
    return {
      symbol, kind: knownAsset(symbol).kind, roundTrips: trips,
      winRate: 24 + rand() * 70,
      matched: row.matchedVolume * (0.05 + rand() * 0.5),
      realized: Math.round(per * trips * (rand() * 2.2 - 0.5) * 100) / 100,
    };
  }).sort((/** @type {{matched: number}} */ a, /** @type {{matched: number}} */ b) => b.matched - a.matched);
