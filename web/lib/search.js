...
import {
  ICONS, assetTile, compact, icon, node, pct, price, shortAddr, signed,
} from './format.js';
...
function defaultPick(/** @type {'asset' | 'trader'} */ kind, /** @type {string} */ id) {
  location.href = kind === 'asset'
    ? `/asset?symbol=${encodeURIComponent(String(id).toUpperCase())}`
    : `/leaderboard?address=${encodeURIComponent(String(id).toLowerCase())}`;
}
...
function assetRow(a, i) {
  ...
  const priceEl = node('span', 'mono search-row-price', price(a.price));
  // The last trade, not a mid or a quote — the same figure the asset page carries, and the
  // same caveat, in the only place a row this size has for one.
  priceEl.title = 'Last trade · not a mid or a quote';
  figs.append(priceEl);
