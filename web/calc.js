...
async function render() {
  ...
  if (!(size > 0) || !Number.isFinite(size)) {
    body.replaceChildren(node('p', 'empty', 'Enter a position size to see the distribution.'));
    return;
  }
  ...
  if (outcomes.length === 0) {
    body.replaceChildren(node('p', 'empty',
      `No ${days}-day windows for this pool at this range. Try a shorter holding period.`));
  ...
  // The distribution comes first. The summary figures sit under it, not over it.
  ...
  hist.setAttribute('aria-label',
    `Distribution of net return versus holding across ${outcomes.length} windows. ` +
    `Fifth percentile ${pct(p5, 2)}, median ${pct(p50, 2)}, ninety-fifth percentile ${pct(p95, 2)}. ` +
  ...
  counts.forEach((/** @type {number} */ c, /** @type {number} */ i) => {
    ...
    const col = node('div', `hist-col ${centre < 0 ? 'hist-col--low' : 'hist-col--high'}`);
    col.style.height = `${tallest ? (c / tallest) * 100 : 0}%`;
    col.title = `${c} window${c === 1 ? '' : 's'} near ${pct(centre, 2)}`;
    hist.append(col);
  });
  const axis = node('div', 'hist-axis');
  axis.append(node('span', '', pct(min, 1)), node('span', '', '0%'), node('span', '', pct(max, 1)));
  body.append(hist, axis);

  const quantiles = node('div', 'quantiles');
  for (const [key, value] of [
    ['5th percentile', pct(p5, 2)],
    ['median', pct(p50, 2)],
    ['95th percentile', pct(p95, 2)],
  ...
  head.style.marginTop = 'var(--s-8)';
  for (const [key, value] of [
    ['median window, fees', pct(median.fees, 2)],
    ['median window, vs holding', pct(median.il, 2)],
    ['median window, in range', pct(median.inRange, 0)],
    ['on', money(size)],
  ]) {
  ...
  el('result-note').textContent =
    `${outcomes.length} windows of ${days} days, ±${width.pct === null ? 'full range' : `${width.pct}%`}. ` +
    `Fee estimate carries at most ${(worstKernelError * 100).toFixed(2)}% relative error.`;
}
...
function renderMethod() {
  ...
  body.replaceChildren();
  for (const text of [
    index.how_to_use.net_vs_hodl_pct === undefined ? '' : `Net versus holding is ${index.how_to_use.net_vs_hodl_pct}.`,
    `The impermanent-loss term ${index.how_to_use.il_is_size_independent}.`,
    `Fees are ${index.how_to_use.fees}. ${index.how_to_use.kernel_error}.`,
    `${prov.fee_model.name}: ${prov.fee_model.what}. ${prov.fee_model.price_taker}.`,
