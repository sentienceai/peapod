...
const NAV = [
  ...
  { id: 'copy', label: 'Copy trade', href: '/copy-trade.html' },
  { id: 'markets', label: 'Markets', href: '/asset.html?symbol=TSLA' },
];
...
export function mountChrome(host, opts = {}) {
  ...
  search.onclick = () => openSearch({ onPick: opts.onSearch });
  host.append(search);
  ...
  const wallet = node('button', 'btn btn-primary', 'Connect wallet');
  /** @type {HTMLButtonElement} */ (wallet).type = 'button';
  wallet.onclick = () => {
    // No wallet flow exists yet, and a button that opens a broken one is worse than a button
    // that says what it is waiting for.
    wallet.textContent = 'Wallets land with execution';
    wallet.disabled = true;
    setTimeout(() => { wallet.textContent = 'Connect wallet'; wallet.disabled = false; }, 2200);
  };
  host.append(wallet);
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === '/' || (String(e.key).toLowerCase() === 'k' && (e.metaKey || e.ctrlKey))) {
      e.preventDefault();
      openSearch({ onPick: opts.onSearch });
    }
  });
}
