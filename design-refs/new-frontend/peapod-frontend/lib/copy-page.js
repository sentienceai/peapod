 ...
 * it has instead is the win-rate chance band (see chanceBand() below), which is a mark on a
...
/**
 * The 95% band a fair coin (p = 0.5) would land a win RATE in after `trips` closes, and
 * whether this row's own win rate falls outside it — the same test lib/profile.js runs on one
 * trader, exported here so the grid can run it on sixteen at once without a second copy of the
 * maths silently drifting from the first.
 *
 * NORMAL APPROXIMATION WITH CONTINUITY CORRECTION, not the exact binomial (Clopper-Pearson)
 * interval — the exact interval needs a beta-quantile solver, real machinery to ship for one
 * mark on one card. At the round-trip counts this board actually has (single digits up to a
 * few dozen closes), the normal curve is a coarse stand-in for a discrete count; the +0.5
 * continuity correction keeps the band from running artificially narrow exactly where `trips`
 * is smallest — which is exactly where a false "beyond chance" verdict would be least
 * deserved. It is still an approximation: the edge of the shaded band is a soft one, not a
 * p-value, and every caller says so in its footnote rather than implying more precision than a
 * normal curve owes a dozen coin flips.
 * @param {number} wins @param {number} trips
 */
export function chanceBand(wins, trips) {
  if (!Number.isFinite(trips) || trips <= 0) return { lo: 0, hi: 1, obs: 0, beyond: false };
  const z = 1.96; // 95%, two-sided
  const halfWidth = z * Math.sqrt(trips * 0.25) + 0.5; // count units, continuity-corrected
  const lo = clamp((trips * 0.5 - halfWidth) / trips, 0, 1);
  const hi = clamp((trips * 0.5 + halfWidth) / trips, 0, 1);
  const obs = clamp(wins / trips, 0, 1);
  return { lo, hi, obs, beyond: obs < lo || obs > hi };
}
...
function chanceRow(row) {
  const { lo, hi, obs, beyond } = chanceBand(row.wins, row.roundTrips);
  ...
  band.style.left = `${(lo * 100).toFixed(1)}%`;
  band.style.width = `${((hi - lo) * 100).toFixed(1)}%`;
  const mark = node('div', 'ev-mark');
  mark.style.left = `${(obs * 100).toFixed(1)}%`;
  track.append(band, node('div', 'ev-null'), mark);
  wrap.append(track);

  const verdict = node('div', `ev-verdict${beyond ? ' ev-verdict-strong' : ''}`,
    beyond ? 'Beyond chance' : 'Within chance');
  wrap.append(verdict);

  const foot = node('div', 'ev-foot',
    `${row.wins} of ${row.roundTrips} closes · chance ${Math.round(lo * 100)}–${Math.round(hi * 100)}%`);
  foot.title = '95% band a fair coin (p = 0.5) would land in at this many closes — a normal '
    + 'approximation with continuity correction, not an exact interval.';
  wrap.append(foot);
  return wrap;
}
...
function buildCard(row) {
  ...
  const copyBtn = /** @type {HTMLButtonElement} */ (node('button', 'btn-copy', 'Copy'));
  copyBtn.type = 'button';
  copyBtn.setAttribute('aria-label', `Copy trade ${shortAddr(row.address)}`);
...
function onSearchPick(kind, id) {
  ...
  else location.href = `/asset.html?symbol=${encodeURIComponent(id.toUpperCase())}`;
}
...
async function init() {
  ...
  const h1 = node('h1');
  h1.append(document.createTextNode('Your portfolio, '), node('em', undefined, 'on autopilot.'));
  heroText.append(h1);
  heroText.append(node('p', undefined,
    'Follow profitable Robinhood Chain traders. Every trade they make is copied to your '
    + 'wallet in real time, inside the limits you set.'));
